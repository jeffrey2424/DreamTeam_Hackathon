BEGIN
    CREATE TEMP TABLE _SESSION.tmpExportTable AS
      ...
      WITH ngram_counts AS
          ...
          WHERE ((DATE(date) BETWEEN DATE_SUB(@run_date, INTERVAL 1 YEAR) AND @run_date) AND LANG = "en")
          UNION ALL
      ...
      INNER JOIN
          ...
          WHERE LANG = "en" AND (DATE(date) BETWEEN DATE_SUB(@run_date, INTERVAL 1 YEAR) AND @run_date) AND mid IN (SELECT mid FROM `hackathon-team-10.companies.companies_list`)
          ) AS t3
      ...
      INNER JOIN
          ...
          WHERE LANG = "en" AND (DATE(date) BETWEEN DATE_SUB(@run_date, INTERVAL 1 YEAR) AND @run_date)
          ) AS t4
    ...
    EXPORT DATA
      OPTIONS(
        uri='gs://hackathon-team-10-gdelt-output/gdelt-history-*.csv',
        format='CSV',
        overwrite=true,
        header=false,
        field_delimiter=',')
