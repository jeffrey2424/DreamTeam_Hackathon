BEGIN
    CREATE TEMP TABLE _SESSION.tmpExportTable AS
      ...
      WITH ngram_counts AS
          ...
          WHERE DATE(date) = @run_date AND ((date BETWEEN TIMESTAMP_ADD(@run_time, INTERVAL -15 MINUTE) AND @run_time) AND LANG = "en")
          UNION ALL
      ...
      INNER JOIN
          ...
          WHERE (DATE(date) = @run_date AND LANG = "en" AND (date BETWEEN TIMESTAMP_ADD(@run_time, INTERVAL -15 MINUTE) AND @run_time) AND mid IN (SELECT mid FROM `hackathon-team-10.companies.companies_list`))
          ) AS t3
      ...
      INNER JOIN
          ...
          WHERE DATE(date) = @run_date AND LANG = "en" AND (date BETWEEN TIMESTAMP_ADD(@run_time, INTERVAL -15 MINUTE) AND @run_time)
          ) AS t4
