EXPORT DATA
  OPTIONS(
    uri='gs://hackathon-team-10-gdelt-output/latest-*.csv',
    format='CSV',
    overwrite=true,
    header=false,
    field_delimiter=',')
AS SELECT * FROM
    ...
    INNER JOIN
        ...
        WHERE (LANG = "en" AND DATE(date) = "2022-04-06" AND mid IN (SELECT mid FROM `hackathon-team-10.companies.companies_list`))
        ) AS t3
