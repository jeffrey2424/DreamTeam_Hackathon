...
SELECT mid, name, url, date, relevant_mentions/total_words AS corpus_score, sentiment_score, sentiment_magnitude, sentiment_salience
FROM
    (
...
INNER JOIN
    ...
    WHERE (avgSalience > 0.5 AND LANG = "en" AND mid IN (SELECT mid FROM `hackathon-team-10.companies`))
    ) AS t3
ON (t1.date = t3.date2) AND (t1.url = t3.url2)
